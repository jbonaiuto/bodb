...
def generate_connectivity_diagram(graphTool, connectionSEDs, user, ext=''):
    ...
    connDiagram = os.path.join('export', '%s.%s.png' % (user.username,ext))
    png_path = os.path.join(settings.MEDIA_ROOT, connDiagram)
    os.system('%s -Tcmapx -o%s -Tpng -o%s %s' % (graphTool, map_path, png_path, dot_path))
    FILE = open(map_path, 'r')
    map = FILE.read()
    FILE.close()
    return connDiagram, map
